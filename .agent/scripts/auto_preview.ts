 ...
 *
 * Usage:
 *   npx tsx auto_preview.ts start [port]
 *   npx tsx auto_preview.ts stop
 *   npx tsx auto_preview.ts status
 */
...
import { savePid, loadPid, deletePidFile, isRunning, killProcessTree } from './utils/process-manager.ts';
 ...
 */
async function getStartCommand(root: string): Promise<string[] | null> {
    const pkgFile = join(root, 'package.json');

    try {
        const data: PackageJson = JSON.parse(await readFile(pkgFile, 'utf-8'));
...
async function stopServer(): Promise<void> {
    ...
    try {
        ...
        console.log(`❌ Error stopping server: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
        await deletePidFile(PID_FILE);
    }
}
