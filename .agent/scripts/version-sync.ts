...
// 1.5 Count actual stats from directories
let actualSkillCount = 53;
let actualWorkflowCount = 19;
let actualAgentCount = 21;
try {
  const skillsDir = path.join(AGENT_DIR, 'skills');
  if (fs.existsSync(skillsDir)) {
    const skillDirs = fs.readdirSync(skillsDir)
      .filter(f => fs.statSync(path.join(skillsDir, f)).isDirectory() && fs.existsSync(path.join(skillsDir, f, 'SKILL.md')));
    actualSkillCount = skillDirs.length;
    // Count agents from AGENTS.md presence
    actualAgentCount = skillDirs
      .filter(f => fs.existsSync(path.join(skillsDir, f, 'AGENTS.md')))
      .length;
  }
  const workflowsDir = path.join(AGENT_DIR, 'workflows');
  if (fs.existsSync(workflowsDir)) {
    actualWorkflowCount = fs.readdirSync(workflowsDir)
      .filter(f => f.endsWith('.md'))
      .length;
  }
  console.log(`\n📊 Detected: ${actualSkillCount} Skills, ${actualAgentCount} Agents, ${actualWorkflowCount} Workflows`);
} catch (e) {
  console.error('Failed to count stats:', e);
}
...
if (fs.existsSync(extPkgPath)) {
  ...
  const expectedDesc = `PikaKit Engine v${newVersion} — Autonomous execution engine. Auto-compiles IDE diagnostics into persistent AI skills and project knowledge.`;
...
// 7d. Sync @version JSDoc in global.d.ts
const globalDtsPath = path.resolve(AGENT_DIR, 'global.d.ts');
if (fs.existsSync(globalDtsPath)) {
  let content = fs.readFileSync(globalDtsPath, 'utf-8');
  const versionTagRegex = /( \* @version )\d+\.\d+\.\d+/;
  if (versionTagRegex.test(content)) {
    const newContent = content.replace(versionTagRegex, `$1${newVersion}`);
    if (content !== newContent) {
      fs.writeFileSync(globalDtsPath, newContent, 'utf-8');
      console.log(`✅ Updated @version in global.d.ts to v${newVersion}`);
      updatedFiles++;
    }
  }
}

// 7e. Sync @version JSDoc in all .agent/skills/ scripts (.js and .ts)
const skillsScriptsDir = path.join(AGENT_DIR, 'skills');
if (fs.existsSync(skillsScriptsDir)) {
  const skillScriptFiles = scanDirectory(skillsScriptsDir, '.js')
    .concat(scanDirectory(skillsScriptsDir, '.ts'));
  for (const filePath of skillScriptFiles) {
    let content = fs.readFileSync(filePath, 'utf-8');
    const versionTagRegex = /( \* @version )\d+\.\d+\.\d+/;
    if (versionTagRegex.test(content)) {
      const newContent = content.replace(versionTagRegex, `$1${newVersion}`);
      if (content !== newContent) {
        fs.writeFileSync(filePath, newContent, 'utf-8');
        console.log(`✅ Updated @version in ${path.relative(ROOT_DIR, filePath)} to v${newVersion}`);
        updatedFiles++;
      }
    }
  }
...
// 8. Update UI stats in pikakit CLI (add-agent-skill-kit)
const cliUiPath = path.resolve(ROOT_DIR, '../add-agent-skill-kit/bin/lib/ui.js');
if (fs.existsSync(cliUiPath)) {
  let content = fs.readFileSync(cliUiPath, 'utf-8');
  // Need to escape backticks manually in replacer if it's regex or use string replace carefully
  const statsRegex = /v\$\{version\} · \d+ Skills · \d+ Workflows · PikaKit\.com/g;
  const newStats = `v\${version} · ${actualSkillCount} Skills · ${actualWorkflowCount} Workflows · PikaKit.com`;
  
  const newerContent = content.replace(statsRegex, newStats);
  if (content !== newerContent) {
    fs.writeFileSync(cliUiPath, newerContent, 'utf-8');
    console.log(`✅ Updated CLI stats in ui.js to ${actualSkillCount} Skills, ${actualWorkflowCount} Workflows`);
    updatedFiles++;
  }
}

// 9. Update stats in READMEs and Package descriptions
const pathsToUpdateStats = [
  path.join(ROOT_DIR, 'README.md'),
  path.resolve(ROOT_DIR, '../add-agent-skill-kit/README.md'),
  path.join(ROOT_DIR, 'package.json'),
  path.resolve(ROOT_DIR, '../add-agent-skill-kit/package.json'),
  path.join(AGENT_DIR, 'rules', 'GEMINI.md'),
];

for (const p of pathsToUpdateStats) {
  if (fs.existsSync(p)) {
    let content = fs.readFileSync(p, 'utf-8');
    let changed = false;
    
    // Markdown Badges: 53_skills
    const skillBadgeRegex = /(\d+)_skills/g;
    if (skillBadgeRegex.test(content)) {
      content = content.replace(skillBadgeRegex, `${actualSkillCount}_skills`);
      changed = true;
    }
    // Markdown Badges: 19_workflows
    const wfBadgeRegex = /(\d+)_workflows/g;
    if (wfBadgeRegex.test(content)) {
      content = content.replace(wfBadgeRegex, `${actualWorkflowCount}_workflows`);
      changed = true;
    }
    
    // Markdown Text mentions: "53 battle-tested"
    const descriptiveRegex = /(\d+) battle-tested engineering capabilities/g;
    if (descriptiveRegex.test(content)) {
      content = content.replace(descriptiveRegex, `${actualSkillCount} battle-tested engineering capabilities`);
      changed = true;
    }
    
    // Markdown/Other footer references: "· 53 Skills · 19 Workflows"
    const footerRegex = /\· (\d+) Skills \· (\d+) Workflows/g;
    if (footerRegex.test(content)) {
      content = content.replace(footerRegex, `· ${actualSkillCount} Skills · ${actualWorkflowCount} Workflows`);
      changed = true;
    }
    
    // GEMINI.md bullet-style: "53 Skills • 21 Agents • 19 Workflows"
    const bulletRegex = /(\d+) Skills • (\d+) Agents • (\d+) Workflows/g;
    if (bulletRegex.test(content)) {
      content = content.replace(bulletRegex, `${actualSkillCount} Skills • ${actualAgentCount} Agents • ${actualWorkflowCount} Workflows`);
      changed = true;
    }
    
    // Package.json description updates: "51 skills, 18 workflows"
    const pkgDescRegex = /(\d+) skills, (\d+) workflows/gi;
    if (pkgDescRegex.test(content)) {
      content = content.replace(pkgDescRegex, `${actualSkillCount} skills, ${actualWorkflowCount} workflows`);
      changed = true;
    }
    
    if (changed) {
      fs.writeFileSync(p, content, 'utf-8');
      console.log(`✅ Updated stats in ${path.relative(ROOT_DIR, p)}`);
      updatedFiles++;
    }
  }
}
