...
Context Engineering provides structured decision frameworks for managing LLM context windows: token budget monitoring, four-bucket reduction strategy (Write/Select/Compress/Isolate), degradation pattern detection, compression technique selection, and multi-agent context isolation. The skill operates as an expert knowledge base that produces context management decisions â€” it does not modify context windows or execute compression.
...
| G2 | Compression with quality preservation | 50â€“70% token reduction; < 5% quality loss |
...
Context: {
  current_utilization: number | null  # 0.0â€“1.0 (percentage of window used)
...
Data: {
  ...
  strategy: {
    ...
    actions: Array<{
      action: string
      expected_reduction: string  # e.g., "30â€“50%"
  ...
  compression: {
    ...
    steps: Array<string>
    target_reduction: string  # "50â€“70%"
...
- Compression target is fixed: 50â€“70% reduction with < 5% quality loss.
- Multi-agent overhead is fixed at ~15x baseline per sub-agent.
- Bucket selection order: Write â†’ Select â†’ Compress â†’ Isolate (escalating intervention).
...
- Compression target (50â€“70%) is achievable for most content types.
...
| Invalid utilization range | Return error to caller | Supply value 0.0â€“1.0 |
...
| Fixed compression target | 50â€“70% reduction; < 5% quality loss |
| Fixed bucket escalation order | Write â†’ Select â†’ Compress â†’ Isolate |
...
| Out-of-range utilization | Return `ERR_INVALID_RANGE` | Supply value 0.0â€“1.0 |
...
| `ERR_INVALID_RANGE` | Validation | No | Utilization value outside 0.0â€“1.0 |
...
| Utilization critical (â‰¥ 0.80) | WARN | utilization, strategy_bucket |
...
| `context.utilization.distribution` | Histogram | 0.0â€“1.0 |
...
- Utilization values are validated to 0.0â€“1.0 range.
...
| Compression overshoot (> 5% quality loss) | Low | Information loss | Fixed 50â€“70% target with quality limit |
...
| Prerequisites documented | âœ… | LLM context window knowledge, token counting |
| When to Use section | âœ… | Utilization-based decision table |
| Core content matches skill type | âœ… | Expert type: decision trees, thresholds, patterns |
| Troubleshooting section | âœ… | Degradation patterns with fixes |
| Related section | âœ… | Cross-links to lifecycle-orchestrator, system-design |
...
|----------|-------|--------|
| **Functionality** | Four-bucket strategy (Write/Select/Compress/Isolate) | âœ… |
| **Functionality** | 4 degradation patterns with fixes | âœ… |
| **Functionality** | 3 compression techniques | âœ… |
| **Functionality** | 4 multi-agent isolation patterns | âœ… |
| **Functionality** | Fixed utilization thresholds (70%/80%) | âœ… |
...
| **Failure** | No silent fallback; all failures return structured errors | âœ… |
| **Failure** | Zero internal retries | âœ… |
| **Determinism** | Fixed thresholds, fixed bucket order, fixed compression target | âœ… |
| **Security** | No context content access; metadata-only operation | âœ… |
